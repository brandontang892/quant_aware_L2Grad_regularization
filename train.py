    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        if isinstance(regularizer, VarianceNorm) or isinstance(regularizer, InverseVarianceNorm):
            ...
            if is_test == False:
                ...
                create_graph_required =  isinstance(regularizer, LPGrad) #not (regularizer == None)
    ...
    with wandb.init(project='RegulQuant', entity='womeiyouleezi', config=args):
        ...
        for epoch in range(0, epochs):
            ...
            if epoch < first_regularized_epoch:
                ...
                train_loss_wandb, train_acc_wandb, train_reg_wandb = run_epoch(net, trainloader, criterion, regularizer, optimizer, epoch, lmbda)
                test_loss_wandb, test_acc_wandb, test_reg_wandb = run_epoch(net, testloader, criterion, regularizer, optimizer, epoch, lmbda, True)
...
#                 torch.save(net.state_dict(), f"{file_name}_best.pt")
            ...
            wandb.log({"train_loss": train_loss_wandb, "test_loss": test_loss_wandb, 
                       ...
                       "train_"+str(regularizer_name): train_reg_wandb, "test_"+str(regularizer_name): test_reg_wandb}) 
